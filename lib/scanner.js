...
function walkDirectory(dir, fileList, ignoredPatterns) {
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      walkDirectory(path.join(dir, entry.name), fileList, ignoredPatterns);
...
module.exports = function(projectPaths, ignoredPatterns, regexSource, regexFlags) {
  ...
  (async () => {
    ...
    for (const projectPath of projectPaths) {
      try {
        const files = [];
        walkDirectory(projectPath, files, ignoredPatterns);
